...
class IWB_Properties {
    ...
    private function encode($t, $v) {
        ...
        switch (strtolower($t)) {
            ...
            case 'f':
                ...
                break;
            case 'd':
                $v=$iwb->Utils->formatDatetime($v);
                break;
            case 'b':
                $v=($iwb->Utils->isTrue($v) ? 'true' : 'false');
                break;
            case 'a':
    ...
    public function getFile($key, $default=FALSE) {
        ...
        if(is_dir($file)) {
            ...
            $file=str_replace("/", DIRECTORY_SEPARATOR, $file);
            if(!$iwb->Utils->endsWith($file, DIRECTORY_SEPARATOR)) {
