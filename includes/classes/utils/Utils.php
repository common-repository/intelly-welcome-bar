...
class IWB_Utils {
    ...
    function query($query, $args=NULL) {
        ...
        if (!is_array($result) || count($result) == 0) {
            ...
            } elseif ($query == IWB_QUERY_POST_TYPES) {
        ...
        if ($args['all']) {
            $first=array();
            $first[]=array('id'=>-1, 'name'=>'['.$iwb->Lang->L('All').']', 'url'=>'');
            $result=array_merge($first, $result);
        }
        if ($args['select']) {
            $first=array();
            $first[]=array('id'=>0, 'name'=>'['.$iwb->Lang->L('Select').']', 'url'=>'');
            $result=array_merge($first, $result);
        }
    ...
    public function formatDatetime($time='now') {
        return $this->formatCustomDate($time, IWB_Utils::FORMAT_DATETIME);
    }
    public function formatCompactDatetime($time='now') {
        return $this->formatCustomDate($time, IWB_Utils::FORMAT_COMPACT_DATETIME);
    }
    public function formatDate($time='date') {
        return $this->formatCustomDate($time, IWB_Utils::FORMAT_DATE);
    }
    ...
    public function formatTime($time='now') {
        return $this->formatCustomTime($time, IWB_Utils::FORMAT_TIME);
    }
    public function formatSqlDatetime($time='now') {
        return $this->formatCustomDate($time, IWB_Utils::FORMAT_SQL_DATETIME);
    }
    public function formatSqlDate($time='date') {
        return $this->formatCustomDate($time, IWB_Utils::FORMAT_SQL_DATE);
    }
    public function formatSqlTime($time='now') {
        return $this->formatCustomTime($time, IWB_Utils::FORMAT_SQL_TIME);
    }
    ...
    public function parseDateToArray($date) {
        ...
        if($iwb->Utils->endsWith($date, 'AM')) {
            $date=substr($date, 0, strlen($date)-2);
            $date=trim($date);
        } elseif($iwb->Utils->endsWith($date, 'PM')) {
            $date=substr($date, 0, strlen($date)-2);
            $date=trim($date);
            $pm=TRUE;
        }
        ...
        if(count($date)==1) {
            ...
            $result=$iwb->Utils->parseArgs($a1, $a2);
        }
    ...
    public function parseDateToTime($date) {
        ...
        $a=$iwb->Utils->parseArgs($result, $defaults);
    ...
    function toListArrayFromClass($array, $id=FALSE, $value=FALSE) {
        ...
        if($array!==FALSE && count($array)>0) {
            foreach($array as $k=>$v) {
                if($id!==FALSE) {
                    $k=$iwb->Utils->get($v, $id);
                }
                if($value!==FALSE) {
                    $v=$iwb->Utils->get($v, $value);
                }
    ...
    function toFormatListArrayFromListObjects($array, $idField, $textFormat) {
        ...
        if($array!==FALSE && count($array)>0) {
            ...
            $columns=array();
            if(is_object($item)) {
                $columns=$iwb->Dao->Utils->getColumns($item);
        ...
        foreach($array as $instance) {
            ...
            foreach($colors as $color=>$when) {
                ...
                foreach($when['conditions'] as $conditionKey=>$conditionValue) {
                    $conditionValue=$iwb->Utils->toArray($conditionValue);
        ...
        foreach ($options as $k=>$v) {
            ...
            if (isset($defaults[$k])) {
                if($this->isAssociativeArray($v)) {
                    ...
                    $v=$iwb->Utils->toArray($v);
                    $old=$defaults[$k];
                    $old=$iwb->Utils->toArray($old);
    ...
    function remotePost($action, $data = '') {
        ...
        $data['secret']='WYSIWYG';
        $response = wp_remote_post(IWB_INTELLYWP_ENDPOINT.'?iwpm_action='.$action, array(
            'method'=>'POST'
            , 'timeout'=>20
            , 'redirection'=>5
            , 'httpversion'=>'1.1'
            , 'blocking'=>TRUE
            , 'body'=>$data
            , 'user-agent'=>IWB_PLUGIN_NAME.'/'.IWB_PLUGIN_VERSION.'; '.get_bloginfo('url')
        ));
        $data = json_decode(wp_remote_retrieve_body($response), TRUE);
        if (is_wp_error($response) || wp_remote_retrieve_response_code($response) != 200
            || !isset($data['success']) || !$data['success']
        ) {
            $iwb->Log->error('ERRORS SENDING REMOTE-POST ACTION=%s DUE TO REASON=%s', $action, $response);
            $data = FALSE;
        } else {
            $iwb->Log->debug('SUCCESSFULLY SENT REMOTE-POST ACTION=%s RESPONSE=%s', $action, $data);
        }
        return $data;
    }
    function remoteGet($uri, $options) {
        ...
        //$uri=str_replace('https://', 'http://', $uri);
            ...
            $iwb->Log->debug('REMOTEGET: URI=%s', $uri);
        $response=FALSE;wp_remote_get($uri, $args);
        if ($response!==FALSE && !is_wp_error($response)) {
                // decode the license data
                $body=wp_remote_retrieve_body($response);
            if($body!==FALSE && $body!='') {
                    $iwb->Log->debug('REMOTEGET: RETRIEVEBODY=%s', $body);
                    $result=json_decode($body);
                    if(!$result) {
                        $iwb->Log->error('REMOTEGET: UNDECODABLE BODY');
                        if(strpos($body, 'debug')!==FALSE || strpos($body, 'fatal')!==FALSE || strpos($body, 'warning')!==FALSE || strpos($body, 'error')!==FALSE) {
                            $iwb->Options->pushErrorMessage('XdebugException');
                        }
                    }
                } else {
                    $iwb->Log->debug('REMOTEGET: RETRIEVEBODY ERROR');
                    $result=FALSE;
                }
        } else {
            $iwb->Log->debug('REMOTEGET: RESULT=ERROR');
        }
        if($result===FALSE) {
            $iwb->Log->debug('file_get_contents: URI=%s', $uri);
            ...
            $iwb->Log->debug('file_get_contents: RETRIEVEBODY=%s', $body);
            if($body!==FALSE && $body!='') {
                $iwb->Log->debug('REMOTEGET: RETRIEVEBODY=%s', $body);
                $result=json_decode($body);
                if(!$result) {
    ...
    function isPluginPage() {
        ...
        $result=($this->startsWith($page, IWB_PLUGIN_SLUG));
        return $result;
    }
    function isQsNull($v) {
    ...
    function jsonToClass($json, $class) {
        global $iwb;
        $instance=$iwb->Dao->Utils->getClass($class);
        if($instance=='') {
        ...
        if($iwb->Utils->isArray($json) && !$iwb->Utils->isAssociativeArray($json)) {
            $match=FALSE;
            $result=array();
            foreach($json as $v) {
        ...
        } elseif($iwb->Utils->isAssociativeArray($json) || is_object($json)) {
    ...
    private function jsonToInstance($json, $class) {
        ...
        $match=FALSE;
        $result=FALSE;
        $columns=$iwb->Dao->Utils->getAllColumns($class);
        $instance=$iwb->Dao->Utils->getClass($class);
        ...
        foreach($json as $property=>$value) {
            if(isset($columns[$property])) {
                ...
                if(isset($column['ui-type']) && $column['ui-type']=='array') {
                    ...
                    $value=$iwb->Dao->Utils->decode($class, $property, $value);
                }
            }
            if($iwb->Utils->set($instance, $property, $value)) {
                $match=TRUE;
            }
        }
        if($match) {
    ...
    public function decodeData($data=array()) {
        ...
        switch (strtolower($data['dataType'])) {
            case 'array':
                ...
                break;
            case 'class':
                if(class_exists($data['dataClass']) && $this->startsWith($data['dataClass'], IWB_PLUGIN_PREFIX)) {
    ...
    function cqs($class, $prefix='') {
        global $iwb;
        $result=$iwb->Dao->Utils->qs($class, $prefix);
        return $result;
    }
    ...
    public function mail($to, $subject, $body, $options=array()) {
        ...
        if($options['footer'] && $options['html']) {
            $body .= '<br><hr><a href="'.IWB_WORDPRESS_SITE.'"><img src="'.IWB_SITE_IMAGES_URI.'logos/logo-mail.png" /></a>';
        }
        $result=FALSE;
    ...
    public function sortOptions_Compare($o1, $o2) {
        global $iwb;
        $v1=$iwb->Utils->get($o1, 'text', FALSE);
        if($v1===FALSE) {
            $v1=$iwb->Utils->get($o1, 'name', FALSE);
        }
        $v2=$iwb->Utils->get($o2, 'text', FALSE);
        if($v2===FALSE) {
            $v2=$iwb->Utils->get($o2, 'name', FALSE);
        }

        //to order properly
        if($iwb->Utils->startsWith($v1, '[')) {
            $v1=' '.$v1;
        }
        if($iwb->Utils->startsWith($v2, '[')) {
            $v2=' '.$v2;
        }
    ...
    public function cURL($uri, $method, $options=array()) {
        ...
        try {
            ...
            if($errorCode==0 && isset($info['http_code'])) {
                ...
                } elseif(is_string($response) && $iwb->Utils->startsWith($response, 'No site configured at this address')) {
                    $errorCode=-1;
                    if($errorDescription=='') {
                        $errorDescription=$response;
                    }
